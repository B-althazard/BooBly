...
const mergerCard = card("Preset Merger", [
  ...
  // JSON code editor (syntax highlight + line numbers)
  // Keeps state.rawJsonText as the single source of truth.
  const _liveParse = parseJsonLenient(state.rawJsonText);
  const codeEditor = jsonEditor(state.rawJsonText, {
    placeholder: "Paste JSON here…",
    errorInfo: _liveParse?.errorInfo || null,
    onInput: (t) => {
      state.rawJsonText = t;
      const r = parseJsonLenient(t);
      state.importError = r.error
        ? (r.errorInfo ? `${r.error} (line ${r.errorInfo.line}, col ${r.errorInfo.col})` : r.error)
        : null;
      state.correctedJsonText = r.error ? (r.corrected || null) : null;
      persistDraft();
    },
  });
...
async function init() {
  ...
  state.master = mLS || (await fetchJson("master_file_v1_4_5_Alpha.json").catch(() => null));
